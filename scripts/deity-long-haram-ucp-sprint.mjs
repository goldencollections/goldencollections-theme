...
const PRODUCT_LIMIT_PER_COLLECTION = Number(argValue("--product-limit") || 450);

const PROMPTS = [
  "deity long haram for idol",
  "long haram for god idol",
  "goddess long haram with size",
  "Balaji long haram for idol"
];
...
const TARGET_RE = /\b(long[-\s]*(?:haram|necklace)|deity[-\s]*long[-\s]*(?:haram|necklace)|god(?:dess)?[-\s]*long[-\s]*(?:haram|necklace)|idol[-\s]*long[-\s]*(?:haram|necklace)|dln\s*-?\s*\d+)\b/i;
const STRONG_TARGET_RE = /\b(long[-\s]*(?:haram|necklace)|dln\s*-?\s*\d+)\b/i;
const WRONG_TARGET_RE =
  /\b(crown|mukut|kireedam|kireetam|earring|jhumki|vaddanam|waist|hands|legs|hastham|padam|nose|arch|prabhavali|short[-\s]*(?:haram|necklace)|vagamalai|thomala|bhujalu)\b/i;
...
if (UCP_ONLY) {
  ...
  console.log(`Wrote deity long haram sprint outputs to ${path.relative(root, OUT_DIR)}`);
}
...
function readExistingTriage() {
  return readJson("long-haram-triage.json", { products: [], summary: {} });
}
...
function runUcpBaseline() {
  const baseline = PROMPTS.map((prompt) => {
    ...
    const results = (parsed.result || []).map((result, index) => ({
      ...
      isTarget: isLongHaramTitle(result.title || ""),
      isWrongType: isWrongLongHaramResult(result.title || "")
    }));
    return {
      prompt,
...
function isLongHaramTitle(title) {
  const text = normalizeText(title);
  return STRONG_TARGET_RE.test(text) || (/\bdeity\b/.test(text) && /\bnecklace\b/.test(text) && /\blong\b/.test(text));
}

function isWrongLongHaramResult(title) {
  const text = normalizeText(title);
  if (!isLongHaramTitle(text)) return true;
...
async function discoverCollections() {
  ...
  const seededLive = [...seededHandles]
    ...
    .map((collection) => ({
      ...collection,
      discoveryScore: Math.max(collectionScore(collection, seededHandles), collection.handle === "deity-long-harams" ? 100 : 1)
    }));
...
function discoverLocalCollectionHandles() {
  ...
  const files = [
    ...
    "scripts/apply-deity-long-harams-optimization.mjs",
  ...
  for (const relativeFile of files) {
    ...
    for (const match of text.matchAll(/\b([a-z0-9]+(?:-[a-z0-9]+){1,})\b/g)) {
      ...
      if (/deity-long/.test(handle) || handle === "deity-necklace") handles.add(handle);
    }
  }
  return [...handles];
}
...
function collectionScore(collection, seededHandles) {
  ...
  let score = 0;
  if (collection.handle === "deity-long-harams") score += 100;
  if (seededHandles.has(collection.handle)) score += 10;
  if (/\bdeity\b/.test(text)) score += 2;
  if (/\blong[-\s]*(haram|necklace|necklaces|harams)\b/.test(text)) score += 8;
  ...
  if (collection.handle !== "deity-necklace" && /\bshort[-\s]*(haram|necklace|necklaces|harams)\b/.test(text)) score -= 10;
...
function collectionSignals(collection) {
  const text = collectionText(collection);
  return [
    collection.handle === "deity-long-harams" ? "exact deity-long-harams handle" : "",
    /\bdeity\b/.test(text) ? "deity signal" : "",
    /\blong[-\s]*(haram|necklace|necklaces|harams)\b/.test(text) ? "long haram/necklace signal" : "",
...
async function runTriage(baseline, discovery) {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "long-haram-triage.json"), `${JSON.stringify(triage, null, 2)}\n`);
  fs.writeFileSync(path.join(OUT_DIR, "long-haram-visual-review.json"), `${JSON.stringify(visualReviewImages, null, 2)}\n`);
  fs.writeFileSync(path.join(OUT_DIR, "long-haram-triage.csv"), toCsv(products.map(productCsvRow)));
  fs.writeFileSync(path.join(OUT_DIR, "long-haram-visual-review.csv"), toCsv(visualReviewImages));
  return triage;
}
...
function proposedMeasurementAlt(product) {
  ...
  const label = /chest/i.test(product.title) ? "chest long haram" : "deity long haram";
...
function writeReport(baseline, triage, discovery) {
  ...
  const currentLongHaramTitles = (triage.products || []).filter((product) => /long haram/i.test(product.title || ""));
  const appliedAltUpdates = readJson("applied-long-haram-measurement-alt-updates.json", []);
  const lines = [
    "# Deity Long Haram UCP Sprint",
    "",
    ...
    "## Product Triage",
    "",
    `- Total likely deity long haram products: ${triage.summary?.total ?? 0}`,
    ...
    `- Current products with long-haram title signal: ${currentLongHaramTitles.length}`,
    `- Applied reviewed long-haram measurement image alt updates: ${appliedAltUpdates.length}`,
    "",
    "### Proof Tiers",
    "",
    ...
    "- `tmp/deity-long-haram-ucp-sprint/collection-discovery.json`",
    "- `tmp/deity-long-haram-ucp-sprint/long-haram-triage.json`",
    "- `tmp/deity-long-haram-ucp-sprint/long-haram-triage.csv`",
    "- `tmp/deity-long-haram-ucp-sprint/long-haram-visual-review.json`",
    "- `tmp/deity-long-haram-ucp-sprint/long-haram-visual-review.csv`",
    "- `tmp/deity-long-haram-ucp-sprint/applied-long-haram-measurement-alt-updates.json`",
    "- `tmp/deity-long-haram-ucp-sprint/long-haram-visual-review-contact-sheet.jpg`",
    "",
    "## Next Safe Step",
    "",
  ...
  fs.writeFileSync(path.join(OUT_DIR, "deity-long-haram-ucp-sprint-report.md"), `${lines.join("\n")}\n`);
}

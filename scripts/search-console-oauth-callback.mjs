...
import { requireSearchConsoleEnv, root, saveToken } from "./search-console-lib.mjs";

const { clientId, clientSecret, redirectUri } = requireSearchConsoleEnv();
const logPath = path.join(root, "tmp", "search-console-oauth.log");
const authUrlPath = path.join(root, "tmp", "search-console-auth-url.txt");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error) {
      ...
      res.end(`Search Console authorization failed: ${message}`);
      server.close();
      return;
    }
    ...
    saveToken(token);
    log("Search Console OAuth token saved to tmp/search-console-token.json");
    ...
    res.end(`Search Console OAuth callback failed: ${error.message}`);
    server.close();
  }
});

#!/usr/bin/env python
"""Build a visual review contact sheet for deity crown image positions 2-4."""
...
ROOT = Path.cwd()
OUT_DIR = ROOT / "tmp" / "crown-ucp-sprint"
SOURCE = OUT_DIR / "crown-visual-review.json"
IMAGE_DIR = OUT_DIR / "visual-review-images"
OUT_PATH = OUT_DIR / "crown-visual-review-contact-sheet.jpg"
MAX_IMAGES = 90
THUMB_W = 300
THUMB_H = 300
LABEL_H = 116
...
def main() -> None:
    ...
    thumbs = [render_tile(row) for row in rows]
    if not thumbs:
...
def prioritize(rows: list[dict]) -> list[dict]:
    def key(row: dict) -> tuple[int, int, str, int]:
        # Put UCP-visible products first, then Tier 2, then earlier image positions.
        return (

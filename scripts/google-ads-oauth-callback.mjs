...
import { requireAdsEnv, root, saveToken } from "./google-ads-lib.mjs";

const { clientId, clientSecret, redirectUri } = requireAdsEnv();
const logPath = path.join(root, "tmp", "google-ads-oauth.log");
const authUrlPath = path.join(root, "tmp", "google-ads-auth-url.txt");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error) {
      ...
      res.end(`Google Ads authorization failed: ${message}`);
      server.close();
      return;
    }
    ...
    saveToken(token);
    log("Google Ads OAuth token saved to tmp/google-ads-token.json");
    res.writeHead(200, { "Content-Type": "text/plain" });
    res.end("Google Ads authorization complete. You can close this tab.");
    server.close();
  } catch (error) {
    log(`Callback failure ${error.stack || error.message}`);
    res.writeHead(500, { "Content-Type": "text/plain" });
    res.end(`Google Ads OAuth callback failed: ${error.message}`);
    server.close();
  }
});

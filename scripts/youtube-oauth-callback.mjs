...
import { requireYouTubeEnv, root, saveToken } from "./youtube-lib.mjs";

const { clientId, clientSecret, redirectUri } = requireYouTubeEnv();
const logPath = path.join(root, "tmp", "youtube-oauth.log");
const authUrlPath = path.join(root, "tmp", "youtube-auth-url.txt");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error) {
      ...
      res.end(`YouTube authorization failed: ${message}`);
      server.close();
      return;
    }
    ...
    saveToken(token);
    log("YouTube OAuth token saved to tmp/youtube-token.json");
    ...
    res.end(`YouTube OAuth callback failed: ${error.message}`);
    server.close();
  }
});

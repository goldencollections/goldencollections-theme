...
import { requireMerchantEnv, root, saveToken } from "./merchant-lib.mjs";

const { clientId, clientSecret, redirectUri } = requireMerchantEnv();
const logPath = path.join(root, "tmp", "merchant-oauth.log");
const authUrlPath = path.join(root, "tmp", "merchant-auth-url.txt");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error) {
      ...
      res.end(`Merchant authorization failed: ${message}`);
      server.close();
      return;
    }
    ...
    saveToken(token);
    log("Merchant OAuth token saved to tmp/merchant-token.json");
    res.writeHead(200, { "Content-Type": "text/plain" });
    res.end("Merchant Center authorization complete. You can close this tab.");
    server.close();
  } catch (error) {
    log(`Callback failure ${error.stack || error.message}`);
    res.writeHead(500, { "Content-Type": "text/plain" });
    res.end(`Merchant OAuth callback failed: ${error.message}`);
    server.close();
  }
});
